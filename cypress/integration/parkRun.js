...
describe('park run scrape', function() {
    ...
    it('visits the park run results page', function() {
        cy.visit('https://www.parkrun.org.uk/wayland/results/latestresults/')
        ...
        cy.get('#results').then($el => {
            ...
            cy.get('h2').then(($el)=>{
                ...
                cy.writeFile('data/parkRunData.json', JSON.stringify({
                    title,
                    data: results
                }, null, 2))
            })
...
// [[pos, 1], [name, jon]] ---> { pos: 1, name: Chirs} 